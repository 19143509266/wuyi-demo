...
const DatasourceSet = (props: Props) => {
  ...
  const { name, options, isTree } = props;
  ...
  return (
    <div>
      {options?.length === 0 ? (
        ...
        <Tree treeData={options} blockNode showLine={isTree} titleRender={titleRender} defaultExpandAll />
      )}
      ...
      <Modal title={'添加数据'} open={addOpen} onCancel={handleCancel} onOk={handleOk} centered>
        <Form form={form}>
          <Form.Item label={'label'} name={'label'} rules={[{ required: true, message: '请输入label' }]}>
            <Input />
          </Form.Item>
          <Form.Item label={'value'} name={'value'} rules={[{ required: true, message: '请输入value' }]}>
            <Input />
          </Form.Item>
        </Form>
      </Modal>
